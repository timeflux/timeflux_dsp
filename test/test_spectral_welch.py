...
def test_welch():
    ...
    for space, expected_peak in expected_peaks.items():
        assert node.o.data.loc[dict(space=space)].frequency[
                   node.o.data.loc[dict(space=space)].argmax('frequency')].frequency.values[0] == expected_peak
